...
CREATE VIEW EL_KUELGUE.BI_REPORTE_MAXIMO_AUTOPARTES_V
...
FROM (SELECT COALESCE(F1.SUCURSAL_CODIGO, C1.SUCURSAL_CODIGO) SUCURSAL_CODIGO, 
	  ...
	  FROM (SELECT SUCURSAL_CODIGO, MES, ANIO, SUM(CANTIDAD_FACTURADA) CANTIDAD_FACTURADA
			FROM EL_KUELGUE.BI_FACTURAS_AUTO_PARTES F JOIN EL_KUELGUE.BI_FACTURA_FECHA FECHA ON F.FACTURA_FECHA = FECHA.ID_FACTURA_FECHA
...
FULL OUTER JOIN (SELECT SUCURSAL_CODIGO, MES, ANIO, SUM(CANTIDAD_COMPRADA) CANTIDAD_COMPRADA
				 FROM EL_KUELGUE.BI_COMPRAS_AUTO_PARTES C JOIN EL_KUELGUE.BI_COMPRA_FECHA FECHA ON C.COMPRA_FECHA = FECHA.ID_COMPRA_FECHA
